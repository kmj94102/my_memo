...
class MemoProvider {
  ...
  Future<List<MemoItem>> getMemoList(
      MemoListSearch condition, String keyword) async {
    ...
    final List<Map<String, dynamic>> maps = await db!.query(memoTableName,
        ...
        whereArgs: ['%$keyword%'],
        orderBy:
            condition == MemoListSearch.searchDate ? "timestamp DESC" : "id");
  ...
  Future<MemoItem?> getMemoItem({required int id}) async {
    ...
    List<Map<String, dynamic>> result =
        await db!.query(memoTableName, where: "id = ?", whereArgs: [id]);
  ...
  Future<int> updateMemo({required int id, required MemoItem memoItem}) async {
    final db = await database;
    var result = await db!.update(memoTableName, memoItem.toMap(),
